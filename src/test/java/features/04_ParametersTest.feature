Feature: Parameter Examples
  ...
  Scenario: Path parameter with kraft
    Given url kraftBaseURL + "/allusers/getbyid"
    And path "5"
    When method GET
    Then status 200

  Scenario: Path parameter with kraft - 2 (can also use endpoint with the path keyword.)
  ...
  Scenario: query parameters -1
    * def endPoint = "/allusers/alluser"
    ...
    And match response[4].id == 33
    And match response[4].name == "Sebastian"
